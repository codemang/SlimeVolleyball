...
class GameCore {
  ...
  startGame() {
    ...
    this.players.forEach(player => {
      player.getSocket().on(events.GAME_UPDATE_FROM_CLIENT, (data) => {
        if (!this.serverInputs)  { this.serverInputs = {}; }
        if (!this.serverInputs[data.user_id]) { this.serverInputs[data.user_id] = []; }

        this.serverInputs[data.user_id].push(data)
      });
    });

    this.server_gameLoop();
    this.server_updateLoop();
  }

  server_gameLoop() {
    setInterval(() => { this.runGame(); }, 15)
  }

  server_updateLoop() {
    const gameState = _.reduce(this.players, (memo, player) => {
      memo[player.getUserId()] = player.getData();
      return memo;
    }, {});

    this.io.to(`room-${this.game_id}`).emit(events.GAME_UPDATE_FROM_SERVER, gameState);
    setTimeout(() => { this.server_updateLoop(); }, 45);
  }
  ...
  initializePlayer(url) {
    ...
    this.socket.on(events.JOIN_SUCCESSFUL, (data) => {
      ...
      // Start game and update loop
      this.client_gameLoop();
      this.client_sendUpdatesToServer();
    });
    ...
    this.socket.on(events.GAME_UPDATE_FROM_SERVER, (data) => {
    });

    this.socket.on(events.GAME_UPDATE, (data) => {
      const seqNum = data.seqNum;
      const serverGameState = data.serverGameState;
    });
  }
  ...
  runGame() {
    ...
    if (!this.is_server) {
      _.forEach(this.keyPresses, (pressed, key) => {
        if (pressed === true) {
    ...
    _.each(this.players, player => {
      player.setFrameRate(this.avgFrameDelta);
      player.applyMoves(this.clientInputs);
    });
    ...
    //   this.clientInputs[player.getUserId()]
    // });
  }

  client_sendUpdatesToServer() {
    this.socket.emit(events.GAME_UPDATE_FROM_CLIENT, {
      user_id: this.player.getUserId(),
      user_data: this.player.getData(),
    })
    setTimeout(() => { this.client_sendUpdatesToServer(); }, 45);
  }

  client_receivedUpdatesFromServer(data) {
  }
